...
int main(int argc, char *argv[]) {
...
#endif
    {
        ...
        while (!conf.eof()) {
            ...
            } else if (key.compare("ENABLE_REPS") == 0) {
                bool v;
                conf >> v;
                enable_reps = v;
                std::cerr << "ENABLE_REPS\t\t" << enable_reps << "\n";
    ...
    topo2bdpMap[std::string("fat_k4_400G_OS2")] = 606000;
    topo2bdpMap[std::string("fat_k8_400G_OS2")] = 606000;
    topo2bdpMap[std::string("leaf_spine_128_400G_OS2")] = 404000;
    ...
    for (uint32_t i = 0; i < node_num; i++) {
        if (n.Get(i)->GetNodeType() == 0) {  // is server
            ...
            rdmaHw->SetAttribute("RepsEnable", UintegerValue(enable_reps));
                    ...
                    for (auto j = table1.begin(); j != table1.end(); j++) {
                        ...
                        for (auto next1 : nexts1) {
                            ...
                            if (nexts2.size() == 1 && nexts2[0]->GetId() == swDstId) {
                                ...
                                uint32_t pathId;
                                memcpy(&pathId, path_ports, sizeof(uint32_t)); // 安全复制
