...
topo2bdp = {
    ...
    "fat_k4_100G_OS2": 156000, # 3-tier -> core 400G
...
def main():
    ...
    #tmode(normal/all-to-all/all-reduce)
    parser.add_argument('--tmode', dest='tmode', action='store',
                        default='normal', help="traffic mode (default: normal)")
    #fixed-traffic
    parser.add_argument('--fixed', dest='fixed', action='store',
                        type=int, default=0, help="fixed traffic (default: 0)")
    #reps
    parser.add_argument('--reps', dest='reps', action='store',
                        type=int, default=0, help="enable REPS (default: 0)")
    ...
    flow = "L_{load:.2f}_TMODE_{tmode}_FIXED_{fixed}_CDF_{cdf}_N_{n_host}_T_{time}ms_B_{bw}_flow".format(
        load=hostload, tmode=args.tmode, fixed=args.fixed, cdf=args.cdf, n_host=n_host, time=int(float(args.simul_time)*1000), bw=bw)

    # check the file exists
    if (exists(os.getcwd() + "/config/" + flow + ".txt")):
        print("Input traffic file with load:{load:.2f}, tmode:{tmode}, fixed:{fixed}, cdf:{cdf}, n_host:{n_host} already exists".format(
            load=hostload, tmode=tmode, fixed=fixed, cdf=cdf, n_host=n_host))
    else:  # make the input traffic file
        print("Generate a input traffic file...")
        print("python ./traffic_gen/ai_traffic_gen.py -c {cdf} -n {n_host} -m {tmode} -f {fixed} -l {load} -b {bw} -t {time} -o {output}".format(
        ...
        os.system("python ./traffic_gen/ai_traffic_gen.py -c {cdf} -n {n_host} -m {tmode} -f {fixed} -l {load} -b {bw} -t {time} -o {output}".format(
            cdf=os.getcwd() + "/traffic_gen/" + args.cdf + ".txt",
            n_host=n_host,
            tmode=tmode,
            fixed=fixed,
    ...
    with open("./mix/.history", "a") as history:
        history.write("{simulday},{config_ID},{cc_mode},{lb_mode},{cwh_tx_expiry_time},{cwh_extra_reply_deadline},{cwh_path_pause_time},{cwh_extra_voq_flush_time},{cwh_default_voq_waiting_time},{pfc},{irn},{reps},{has_win},{var_win},{topo},{bw},{cdf},{tmode},{load},{time},{fixed}\n".format(
            simulday=simulday,
            config_ID=config_ID,
            cc_mode=cc_mode,
            lb_mode=lb_mode,
    ...
    if (cc_mode == 1):  # DCQCN
        ...
        config = config_template.format(id=config_ID, topo=topo, flow=flow,
                                        ...
                                        enabled_pfc=enabled_pfc, enabled_irn=enabled_irn, enabled_reps = enabled_reps,
                                        cc_mode=cc_mode,
    ...
    #     flowgen_start_time * 1e9) + int(0.000005 * 1e9)  # warmup
    fct_analysis_time_limit_begin = int(
        flowgen_start_time * 1e9)  # warmup
