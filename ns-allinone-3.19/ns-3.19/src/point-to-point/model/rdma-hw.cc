...
TypeId RdmaHw::GetTypeId(void) {
    static TypeId tid =
        TypeId("ns3::RdmaHw")
            ...
            .AddAttribute("Mtu", "Mtu.", UintegerValue(1000), MakeUintegerAccessor(&RdmaHw::m_mtu),//最大传输单元（默认 1000 字节）
            ...
            .AddAttribute("TargetUtil",
                          ...
                          MakeDoubleChecker<double>())
            // 新增 REP 负载均衡开关属性
            .AddAttribute("RepsEnable", 
                          "Enable/Disable REPS load balancing",
                          UintegerValue(0),  // 默认关闭
                          MakeUintegerAccessor(&RdmaHw::m_enable_reps), // 绑定成员变量
                          MakeUintegerChecker<uint32_t>(0, 1)) // 类型验证
            .AddAttribute(
                "UtilHigh",
...
void RdmaHw::Setup(QpCompleteCallback cb) {
    ...
    for (uint32_t i = 0; i < m_nic.size(); i++) {
        ...
        // 1. 共享数据：将 NIC 的 RDMA 事件队列与 QP 组绑定
        dev->m_rdmaEQ->m_qpGrp = m_nic[i].qpGrp;
        // 2. 设置回调函数
        dev->m_rdmaReceiveCb = MakeCallback(&RdmaHw::Receive, this);      // 接收数据包回调
        dev->m_rdmaLinkDownCb = MakeCallback(&RdmaHw::SetLinkDown, this); // 链路断开回调
        dev->m_rdmaPktSent = MakeCallback(&RdmaHw::PktSent, this);        // 数据包发送完成回调
        // 3. 配置 NIC 参数
        dev->m_rdmaEQ->m_mtu = m_mtu;                                              // 设置 MTU
        dev->m_rdmaEQ->m_rdmaGetNxtPkt = MakeCallback(&RdmaHw::GetNxtPacket, this);// 获取下一数据包回调
    }
    // 4. 设置 QP 完成回调
...
//将 QP 的元组（目标 IP、源端口、目标端口、优先级组）编码为唯一的 64 位键，用于快速查找 QP
...
void RdmaHw::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip,
    ...
    //设置ev
    uint16_t ev = rand() % EVS_SIZE; // 自定义哈希函数
    
    if(m_enable_reps){
        qp = CreateObject<RdmaQueuePair>(pg, sip, dip, ev, dport);
        qp->SetEntropyValue(ev);
    }else{
        qp = CreateObject<RdmaQueuePair>(pg, sip, dip, sport, dport);
    }
    ...
    uint64_t key;
    if(m_enable_reps){
        key = GetQpKey(dip.Get(), ev, dport, pg);
    }else{
        key = GetQpKey(dip.Get(), sport, dport, pg);
    }
...
int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch) {
    ...
    int x = ReceiverCheckSeq(ch.udp.seq, rxQp, payload_size, cnp_check);//检查数据包是否按序到达

    if (x == 1 || x == 2 || x == 6) {  // generate ACK or NACK, 需要生成响应
        qbbHeader seqh;
        seqh.SetSeq(rxQp->ReceiverNextExpectedSeq); // 设置期望的下一个序列号
        seqh.SetPG(ch.udp.pg);
        seqh.SetSport(ch.udp.dport);               // 交换源/目的端口（应答方向）
...
int RdmaHw::ReceiveAck(Ptr<Packet> p, CustomHeader &ch) {
    ...
    // reps部分：解析 ECN 标记
    bool ecnMarked = (ch.GetIpv4EcnBits() == 0x03); // ECN=0x03 表示拥塞
    uint16_t receivedEv = port;
    ...
    if(m_enable_reps){
        // 反馈 ECN 状态到 QP    
        qp->OnAckReceived(ecnMarked, receivedEv);  
    }
...
void RdmaHw::HandleTimeout(Ptr<RdmaQueuePair> qp, Time rto) {
    ...
    if(m_enable_reps){
      // 触发故障检测
      qp->OnFailureDetected();  
      if (qp->isFreezingMode) {
            return; 
        }
    }
