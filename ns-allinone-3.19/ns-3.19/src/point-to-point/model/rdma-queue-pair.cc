...
RdmaQueuePair::RdmaQueuePair(uint16_t pg, Ipv4Address _sip, Ipv4Address _dip, uint16_t _sport,
    ...
    // m_ev = rand() % EVS_SIZE; 
    m_ev = 0; 
    repsBuffer.resize(REPS_BUFFER_SIZE);
    for (auto& elem : repsBuffer) {
        elem.cachedEV = 0;
        elem.isValid = false;
    }
...
void RdmaQueuePair::SetEntropyValue(uint16_t ev) { m_ev = ev; }
...
uint32_t RdmaQueuePair::GetHash_EV(void){
    // 直接使用EV生成哈希值
    return Hash32(reinterpret_cast<const char*>(&m_ev), sizeof(m_ev));
}
...
uint32_t RdmaRxQueuePair::GetHash_EV(void){
    // 直接使用EV生成哈希值
    return Hash32(reinterpret_cast<const char*>(&m_ev), sizeof(m_ev));
}
...
// REPS 选择下一个 EV
uint16_t RdmaQueuePair::GetNextEntropy() {
    uint32_t offset;
    if (numberValidEVs > 0) {
        // 计算最早的有效 EV 
        offset = (head - numberValidEVs + repsBuffer.size()) % repsBuffer.size();
        repsBuffer[offset].isValid = false;
        numberValidEVs--;
    } else {
        // 无有效 EV 时使用 head 位置
        offset = head;
        head = (head + 1) % repsBuffer.size();
    }
    return repsBuffer[offset].cachedEV;
}

// 处理 ACK 反馈
void RdmaQueuePair::OnAckReceived(bool ecnMarked, uint16_t receivedEv) {
    if (ecnMarked) return; 
    RepsBufferElement& current = repsBuffer[head];
    if (!current.isValid) {
        numberValidEVs = (numberValidEVs < 255) ? numberValidEVs + 1 : 255; 
    }
    current.cachedEV = receivedEv;
    ...
    if (isFreezingMode && now_ns > exitFreezingMode) {
        isFreezingMode = false;
...
// 处理故障检测
void RdmaQueuePair::OnFailureDetected() {
    if (!isFreezingMode && exploreCounter == 0) {
        isFreezingMode = true;
        exitFreezingMode = Simulator::Now().GetNanoSeconds() + Seconds(FREEZING_TIMEOUT).GetNanoSeconds(); // 转换为纳秒;
    }
}

void RdmaQueuePair::OnSend(Ptr<Packet> packet, CustomHeader &ch) {
    if (repsBuffer.empty() || (numberValidEVs == 0 && !isFreezingMode) || exploreCounter > 0) {
        m_ev = rand() % EVS_SIZE; 
        exploreCounter = (exploreCounter > 0) ? (exploreCounter - 1) : 0;
    } else {
        m_ev = GetNextEntropy();
    }
    if (ch.l3Prot == 0x11) {// UDP 
        ...
        udpHeader.SetSourcePort(m_ev);    
        packet->AddHeader(udpHeader);      
    } else if (ch.l3Prot == 0x86DD) { // IPv6
        Ipv6Header ipv6Header;
        packet->RemoveHeader(ipv6Header);
        ipv6Header.SetFlowLabel(m_ev);    
