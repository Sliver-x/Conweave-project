...
//REPS常数
#define REPS_BUFFER_SIZE 8
#define EVS_SIZE 65535
#define FREEZING_TIMEOUT 5 // seconds

#define NUM_PKTS_BDP 10
...
class RdmaQueuePair : public Object {
   public:
    ...
    // REPS 算法状态变量
    struct RepsBufferElement {
        uint16_t cachedEV;   // 缓存的熵值
        bool isValid;        // 标记该 EV 是否有效
    };
    std::vector<RepsBufferElement> repsBuffer; // 循环缓冲区
    uint8_t head;            // 8 位头指针
    uint8_t numberValidEVs;  // 8 位有效 EV 计数
    uint32_t exitFreezingMode; // 32 位冻结模式退出时间（纳秒）
    bool isFreezingMode;     // 1 位冻结模式标志
    uint8_t exploreCounter;  // 8 位探索计数器
    uint16_t m_ev; // 熵值（EV）
    ...
    inline bool IsFinishedConst() const { return snd_una >= m_size; }
     ...
     // REPS 算法相关方法
    uint16_t GetNextEntropy();       // 获取下一个 EV
    void OnAckReceived(bool ecnMarked, uint16_t receivedEv);    // 处理 ACK 反馈
    void OnFailureDetected();        // 故障检测
    void OnSend(Ptr<Packet> packet, CustomHeader &ch);
    void SetEntropyValue(uint16_t ev);//设置ev的值
