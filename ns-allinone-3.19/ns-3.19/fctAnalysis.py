...
# 		if len(fct_over_bdp) == 0:
#             avg_over = p50_over = p95_over = p99_over = p999_over = 0
#         else:
#             avg_over = np.average(fct_over_bdp)
#             p50_over = np.percentile(fct_over_bdp, 50)
#             p95_over = np.percentile(fct_over_bdp, 95)
#             p99_over = np.percentile(fct_over_bdp, 99)
#             p999_over = np.percentile(fct_over_bdp, 99.9) if len(fct_over_bdp) >= 1000 else 0
#         outfile_fct_summary.write(
#             "{:5},{:.3f},{:.3f},{:.3f},{:.3f},{:.3f}\n".format(">1BDP", avg_over, p50_over, p95_over, p99_over,
#                                                                p999_over))
# 		outfile_fct_summary.write("#\n#\n#\n#\n#\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='')
    parser.add_argument('-id', '--id', dest='id', required=True, action='store', help="traceId")
    parser.add_argument('-dir', '--dir', dest='dir', default='.', action='store',
                        help="directory of run.py file, default='.'")
    parser.add_argument('-fdir', '--fdir', dest='fdir', default='mix', action='store',
                        help="folder that the output files are located, default=mix")
    parser.add_argument('-bdp', dest='bdp', action='store', required=True,
                        help="1 BDP of this topology, default=104000 (100G with 2-tier)")
    parser.add_argument('-sT', dest='time_limit_begin', action='store', type=int, default=2000000000,
                        help="only consider flows that finish after T, default=2.005*10^9 ns")
    parser.add_argument('-fT', dest='time_limit_end', action='store', type=int, default=100000000000,
                        help="only consider flows that finish before T, default=100 * 10^9 ns")
    ...
    output_fct = dirname + "/" + fdirname + "/output/{id}/{id}_out_fct.txt".format(id=config_ID)
    output_fct_summary = dirname + "/" + fdirname + "/output/{id}/{id}_out_fct_summary.txt".format(id=config_ID)
    output_fct_small_absolute_cdf = dirname + "/" + fdirname + "/output/{id}/{id}_out_fct_small_absolute_cdf.txt".format(
        id=config_ID)
    output_fct_small_slowdown_cdf = dirname + "/" + fdirname + "/output/{id}/{id}_out_fct_small_slowdown_cdf.txt".format(
        id=config_ID)
    output_fct_large_absolute_cdf = dirname + "/" + fdirname + "/output/{id}/{id}_out_fct_large_absolute_cdf.txt".format(
        id=config_ID)
    output_fct_large_slowdown_cdf = dirname + "/" + fdirname + "/output/{id}/{id}_out_fct_large_slowdown_cdf.txt".format(
        id=config_ID)
    output_fct_all_slowdown_cdf = dirname + "/" + fdirname + "/output/{id}/{id}_out_fct_all_slowdown_cdf.txt".format(
        id=config_ID)
    output_fct_all_absolute_cdf = dirname + "/" + fdirname + "/output/{id}/{id}_out_fct_all_absolute_cdf.txt".format(
        id=config_ID)
    ...
    # read lines
    file = "%s" % (output_fct)
    cmd_absolute = "cat %s" % (file) + " | awk '{if ($6>=" + "%d" % time_limit_start + " && $6+$7<" + "%d" % (
        time_limit_end) + ") {print $7/1000, $5} }' | sort -n -k 2"
    print(cmd_absolute)
    output_absolute = subprocess.check_output(cmd_absolute, shell=True)
    cmd_slowdown = "cat %s" % (file) + " | awk '{if ($6>=" + "%d" % time_limit_start + " && $6+$7<" + "%d" % (
        time_limit_end) + ") {print $7/$8<1?1:$7/$8, $5} }' | sort -n -k 2"
    print(cmd_slowdown)
    output_slowdown = subprocess.check_output(cmd_slowdown, shell=True)
    ...
    with open(output_fct_summary, "w") as outfile_fct_summary:
        ...
        for item in res:
            line = "#%.3f %3d" % (item[0] + step / 100.0, item[1])
            i = 1
            line += "\t{:.3f} {:.3f} {:.3f} {:.3f} {:.3f}\n".format(item[i + 1], item[i + 2], item[i + 3], item[i + 4],
                                                                    item[i + 5])
            outfile_fct_summary.write(line)
